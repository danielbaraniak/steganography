...
# Generate images with larger dimensions
...
def main():
    for size in image_sizes:
        ...
        # save with quality=100 to avoid compression artifacts
        original_image.save(f"original_{size[0]}x{size[1]}.jpg", quality=100)
        original_image.save(f"original_{size[0]}x{size[1]}.png", quality=100)

        # for method, method_name in interpolation_methods:
        #     scaled_image = scale_image(original_image, interpolation=method)
        #     cv2.imwrite(f"scaled_{size[0]}x{size[1]}_{method_name}.png", scaled_image)
