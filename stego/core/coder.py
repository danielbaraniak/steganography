...
def modify_blocks(message: bytes, blocks: list[np.ndarray], bit_step: int, alpha: float) -> list[ndarray]:
...
def encode(image: np.ndarray, message_parts: list[bytes], *, coefficients: list[str], alpha: float = 1,
    ...
    for coefficient, message in zip(coefficients, message_parts):
        ...
        decomposition[1][coefficient] = blocking.merge_blocks(modified_blocks, cover.shape)
...
def decode(image: np.ndarray, *, coefficients: list[str], block_size: int = 3, level: int = 2,
           wavelet: str = "haar", **kwargs) -> list[bytes]:
    decomposition = pywt.wavedecn(image, wavelet=wavelet, level=level)
    message_parts = []
...
def get_capacity(image: np.ndarray, *, coefficients: list[str], block_size: int = 3, level: int = 2, **kwargs) -> tuple[
    int, int]:
    """Calculates the capacity of the image in bytes."""
    initial_block_size = block_size * 2 ** level
    height, width = image.shape[:2]
    coefficient_capacity = (height // initial_block_size * width // initial_block_size) // BYTE
    image_capacity = coefficient_capacity * len(coefficients)
    return image_capacity, coefficient_capacity


def message_dispatcher(image: np.ndarray, message: bytes, *, coefficients: list[str], block_size: int = 3,
                       level: int = 2, **kwargs):
    """Divides the message into parts that fit into coefficients of the image."""
    image_capacity, coefficient_capacity = get_capacity(image, block_size=block_size, level=level,
    ...
    message_parts = [new_message[i:i + coefficient_capacity] for i in range(0, len(new_message), coefficient_capacity)]
    message_parts[-1] = message_parts[-1] + b'\x00' * remainder
    return message_parts


def uniform_message_dispatcher(image: np.ndarray, message: bytes, *, coefficients: list[str], block_size: int = 3,
                               level: int = 2, **kwargs):
...
def message_consolidator(image: np.ndarray, message_parts: list[bytes], *, coefficients: list[str], block_size: int = 3,
                         level: int = 2, **kwargs) -> bytes:
